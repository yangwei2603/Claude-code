...
> 本地数据分析工作区，用于春秋航空经营分析、成本分析、预算分析、合同分析、财务报表、合规风控、外航对比、项目管理等数据分析任务。
...
│       ├── index.html                # HTML报告
│       └── charts/                  # 图表输出
├── 航油成本分析/                        # 航油成本分析项目（符合上述结构）
├── CLAUDE.md                         # 本文件
└── README.md                         # 工作区概览
```

**主题分析目录规范**：`data-analysis-local/<主题>/<YYYYMMDD>/`
- 新分析领域建议采用父目录 + 日期子目录结构（与航油成本分析相同）
- 父目录存放模板文件（scripts、metadata 等），子目录存放每次分析任务
- 如用户已在其他位置手工创建目录，以用户实际位置为准，不强制迁移
...
```
data-analysis-local/
└── <主题>-<YYYYMMDD>/           # 分析任务根目录（必须）
    ├── CLAUDE.md                 # 项目说明（必填，记录分析目标、数据源、结论）
    ├── analyze_*.py              # 分析脚本（必须）
    ├── data.json                 # 分析数据（由脚本生成）
...
| 文件 | 用途 |
|------|------|
| `CLAUDE.md` | 分析背景、数据源、核心发现、待解决问题 |
| `analyze_*.py` | 主分析脚本（Python） |
| `index.html` | 交互式 HTML 报告 |

**禁止事项**：
- 不得复用历史目录进行新分析（每次必须新建）
- 分析输出不得存放在工作区根目录
- 临时文件不得污染分析目录
...
分析数据库中已内置元数据表，可直接 SQL 查询：

```sql
-- 表级别元数据
SELECT table_name, description, row_count FROM table_metadata;

-- 字段级别元数据（例：查 t_ads_sfuel_stat_dtl 所有字段）
SELECT field_name, field_type, description
FROM field_metadata
WHERE table_name = 't_ads_sfuel_stat_dtl'
ORDER BY field_name;

-- 关键词搜索字段（例：搜索所有"油耗"相关字段）
SELECT table_name, field_name, description
FROM field_metadata
WHERE description LIKE '%油耗%';
```

| 元数据表 | 说明 | 记录数 |
|---------|------|--------|
| `table_metadata` | 表级别元数据（表名、说明、源文件、行数） | ~10 |
| `field_metadata` | 字段级别元数据（表名、字段名、类型、中文说明） | 473 |

**覆盖范围**：航油成本相关表（t_ads_sfuel_stat_dtl/d/m/route_m、v_dws_flt_opt_stat_d）

**Obsidian 数据字典**（详细业务口径说明）：

```
~/Documents/Obsidian Vault/自动笔记/03-数据资产/01-数据治理/航油成本数据字典/
├── t_ads_sfuel_stat_dtl_数据字典.md    # 航班明细表（91字段）
├── 节油相关表数据字典.md                 # 日表/月表/航线月表/运行视图
├── 航油成本数据口径手册.md              # 核心指标计算口径
...
| Skill | 路径 | 用途 |
|-------|------|------|
| `data-analysis` | `../skills/domain/data-analysis/` | 通用数据分析流程（含财务分析、ML建模） |
