...
用法:
  ...
  python run.py --portable            # 便携模式（首次运行自动创建配置）
"""
...
# ============================================================
# PyInstaller 兼容：获取正确的工作目录
# ============================================================
def _get_app_dir() -> Path:
    """获取应用根目录（兼容 PyInstaller 打包）"""
    if getattr(sys, 'frozen', False):
        # PyInstaller 打包模式：sys._MEIPASS 是临时解压目录
...
# ============================================================
DEFAULT_CONFIG = {
    "source_dir": "~/Documents",  # 默认用户文档目录
    "state_file": "state/organizer_state.json",
    "log_dir": "logs",
    "rollback_dir": "state/rollback",
    "dry_run": True,
    "keep_original": False,
    "scan_days": 7,
...
def print_stats(stats, dry_run: bool):
    ...
    print(f"\n[{mode}] 整理完成")
    print(f"  扫描文件 : {s.get('scanned', 0)}")
    print(f"  待整理   : {s.get('organized', 0)}")
    print(f"  删除文件 : {s.get('deleted', 0)}")
    print(f"  已跳过   : {s.get('skipped', 0)}")
...
def main():
    ...
    mode_group.add_argument("--portable",   action="store_true", help="便携模式（首次运行自动创建配置）")
    ...
    # ---- 便携模式：用户数据目录 = 可执行文件所在目录 ----
    is_portable = getattr(sys, 'frozen', False) or args.portable
    if is_portable:
    ...
    # 便携模式首次运行：自动创建默认配置
    if is_portable and not config_path.exists():
        try:
            config_path.parent.mkdir(parents=True, exist_ok=True)
            with open(config_path, "w", encoding="utf-8") as f:
                json.dump(DEFAULT_CONFIG, f, ensure_ascii=False, indent=2)
            print(f"[便携模式] 首次运行，已创建默认配置: {config_path}")
        except Exception as e:
            print(f"[警告] 无法创建默认配置: {e}")
    ...
    if args.watch:
        ...
        run_watch_mode(config, app_dir)
        return

    # ---- Stats 模式 ----
    if args.stats:
        log_dir = user_data_dir / config.get("log_dir", "logs")
        print_banner()
        show_stats(log_dir)
        return

    # ---- 导入引擎 ----
    # 使用新的 FileOrganizer（organizer/ 包）优先
    # 它集成了 4 级级联分类（关键词→LLM→内容分析→扩展名兜底）
    ...
    organizer.organize(incremental=incremental)
